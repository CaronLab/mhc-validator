...
import pandas as pd
with open(
        '/Data/Development/Validator/validation_experiments/search_with_crap_database/JY_Human/JY_301120_S3_target.pout',
        'r') as f:
...
decoy_pout = pd.read_table(
    '/Data/Development/Validator/validation_experiments/search_with_crap_database/JY_Human/JY_301120_S3_decoy.pout',
    usecols=header, index_col=False)

qs = v.qs[v.y == 1]  # get q-values of targets
roc = np.sum(qs <= qs[:, np.newaxis], axis=1)
plt.plot(qs, roc, ls='none', marker='.', ms=1, label='Validator')
...
# and plot then alongside Validator
qs = v.qs[v.y == 1]  # get q-values of targets
roc = np.sum(qs <= qs[:, np.newaxis], axis=1)
plt.plot(qs, roc, ls='none', marker='.', ms=1, label='Validator')
...
plt.xlabel('q-value')
plt.ylabel('Number of PSMs')
plt.title('Validator q-values vs recalculated Percolator q-values')
plt.xlim((0, 1))
plt.legend()
plt.show()
...
for pin in pin_files:
    ...
    v.train_validation_model(encode_peptide_sequences=True, epochs=15, visualize=False,
    ...
    v.train_validation_model(encode_peptide_sequences=False, epochs=15, visualize=False,
...
venn3_unweighted([set(purcell_peps), set(validator_peps_w_encoding_wo_mhcflurry), set(validator_peps_wo_encoding_wo_mhcflurry)],
      ['Purcell master list', 'Validator with\nencoding', 'Validator without\nencoding'])
plt.title('Validator without MhcFlurry predictions')
plt.show()

venn3_unweighted([set(purcell_peps), set(validator_peps_w_encoding_w_mhcflurry), set(validator_peps_wo_encoding_w_mhcflurry)],
      ['Purcell master list', 'Validator with\nencoding', 'Validator without\nencoding'])
plt.title('Validator with MhcFlurry predictions')
plt.show()
